...
test.describe('Vite React 19 - Source Endpoint', () => {
  test('returns source code for valid file', async ({ request }) => {
    ...
    expect(data.endLine).toBeGreaterThan(data.startLine)
  })
  ...
  test('returns 404 for nonexistent file', async ({ request }) => {
    const response = await request.get('/__rcf/source?file=nonexistent.tsx&line=1')
    expect(response.status()).toBe(404)
  })

  test('returns 400 for missing file parameter', async ({ request }) => {
    const response = await request.get('/__rcf/source?line=1')
    expect(response.status()).toBe(400)
  })
})

test.describe('Vite React 19 - Inspector', () => {
  ...
  test('hover shows overlay with positioning', async ({ page }) => {
...
test.describe('Vite React 19 - Click Copy (Props + Source + XML)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await waitForInspector(page)
    await enableInspector(page)
  ...
  test('click on Card copies Card component info', async ({ page }) => {
...
test.describe('Vite React 19 - Area Selection', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await waitForInspector(page)
    await enableInspector(page)
  ...
  test('S key enters selection mode, drag copies component tree', async ({ page }) => {
