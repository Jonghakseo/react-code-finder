...
const SOURCE_SERVER = 'http://localhost:7799'

test.describe('Next.js React 19 - Source Endpoint', () => {
  test('returns source code for valid file', async ({ request }) => {
    const response = await request.get(`${SOURCE_SERVER}/source?file=pages/index.tsx&line=4&context=5`)
    expect(response.ok()).toBeTruthy()
    const data = await response.json()
    expect(data.file).toBe('pages/index.tsx')
    expect(data.content).toContain('Home')
  })

  test('returns source for component file', async ({ request }) => {
    const response = await request.get(`${SOURCE_SERVER}/source?file=components/Button.tsx&line=27&context=5`)
    expect(response.ok()).toBeTruthy()
    const data = await response.json()
  ...
  test('blocks path traversal', async ({ request }) => {
    const response = await request.get(`${SOURCE_SERVER}/source?file=../package.json&line=1`)
    expect(response.status()).toBe(403)
  })

  test('returns 404 for nonexistent file', async ({ request }) => {
    const response = await request.get(`${SOURCE_SERVER}/source?file=nonexistent.tsx&line=1`)
    expect(response.status()).toBe(404)
  })
})

test.describe('Next.js React 19 - Inspector', () => {
...
test.describe('Next.js React 19 - Click Copy (Props + Source + XML)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await waitForInspector(page)
    await enableInspector(page)
...
test.describe('Next.js React 19 - Area Selection', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await waitForInspector(page)
    await enableInspector(page)
